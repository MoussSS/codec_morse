...
#define MORSE_REFERENCE_DURATION_MS 200
...
static bool encode_morse_blank(void) {
    ...
    cycle++;
    if (cycle >= SPACE_BETWEEN_LETTERS_NB_OF_CYCLE) {
        cycle = 0;
    }
...
static bool sending_char = false;

static bool send_character (char char_to_send) {
    bool busy;
    
    if (sending_char) {
        busy = encode_morse_data();
    } else {
...
void encode_morse_message(void) {
    ...
    switch (step) {
        ...
        case 1:
        set_morse_signal(false);
        if (cycle >= SPACE_BETWEEN_PARTS_NB_OF_CYCLE) {
            cycle = 0;
            step++;
        }
        break;
